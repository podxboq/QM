...
\begin{definition}
    Sea $\maps{L}{\R^n\times\R^n\times\R}{\R}$ una aplicación real y $\maps{\alpha}{I=[t_1, t_2]}{\R^n}$ la trayectoria de una partícula, definimos el funcional \define{acción de L sobre $\alpha$}{acción sobre una trayectoria} a:
    \begin{equation}
        \label{eq:accion}
...
\begin{theorem}
    Una trayectoria $\alpha$ alcanza un valor extremo (máximo o mínimo) en el funcional $S_L(\alpha)$ sí y sólo sí se da la siguiente igualdad llamada \define{ecuación de Euler-Lagrange}{Ecuación de Euler-Lagrange}
    \begin{equation}
...
\begin{definition}
    Diremos que dos trayectorias $\maps{\alpha,\beta}{I=[t_0, t_1]}{\R^n}$ son \define{equivalentes}{trayectorias equivalentes} si $\alpha(t_0)=\beta(t_0)$, $\alpha(t_1)=\beta(t_1)$ y $S_L(\alpha)=S_L(\beta)$.
\end{definition}

Si tenemos dos trayectorias $\maps{\alpha,\beta}{I=[t_1, t_2]}{\R^n}$ equivalentes, la diferencia de sus acciones es cero, por lo tanto $\int_{I} L_\beta(t)-L_\alpha(t)\ dt=0$, es decir, existe una función $\maps{f}{\R}{\R}$ que es la primitiva de $L_\beta-L_\alpha$ tal que:
